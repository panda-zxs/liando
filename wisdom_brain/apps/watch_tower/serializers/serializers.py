...
from wisdom_brain.apps.watch_tower.models import (
    Area, Company,
    Industry, Relation
)
from wisdom_brain.apps.watch_tower.utils import get_area_industry
...
class StatisticsSerializer(serializers.ModelSerializer):
    ...
    top5 = serializers.SerializerMethodField(
        help_text='几个top5的统计')
    revenue_data = serializers.SerializerMethodField(
        help_text='营收数据统计')
    tax_revenue_data = serializers.SerializerMethodField(
        help_text='税收数据统计')
...
class DataSerializer(serializers.ModelSerializer):
    ...
    def get_revenue(cls, data):
        ...
        revenue = Company.objects.filter(
            id__in=company, established_time__year__lte=settings.YEAR
        ).aggregate(Sum('revenue'))
        increase = Company.objects.filter(
            id__in=company, established_time__year=settings.YEAR
        ).aggregate(Sum('revenue'))
        ret = dict()
...
class RevenueDataSerializer(serializers.ModelSerializer):
    ...
    def get_count(data):
        ...
        details = Relation.objects.filter(
            ...
            interval=Case(
                When(Q(company__revenue__lt=10 ** 7),
                     then=Value('0-1')),
    ...
    def get_area(data):
        ...
        all_area_revenue = Relation.objects.filter(
            ...
            account=Sum('company__revenue')
        ).order_by('account').values('account')[0]
...
class AreaTradeLSerializer(serializers.ModelSerializer):
    area_info = serializers.SerializerMethodField(method_name='get_area')
    ...
    @staticmethod
    def get_area(_):
        return Area.objects.filter(deleted=False).values(
            'id', 'name', 'first_id',
            'second_id', 'grade')

    @staticmethod
    def get_industry(_):
        return Industry.objects.filter(deleted=False).values(
